...
if (isset($_SESSION['logged_in']) && !empty($_SESSION['logged_in']) && $_SESSION['logged_in']) {
      ...
      if($results->num_rows > 0) {
        ...
        $userID=$row['userID'];
        $sql = "SELECT * FROM Searches WHERE userID=".$userID." ORDER BY searchID DESC;";
        $results = $mysqli->query($sql);
      } else {
...
<html lang="en" dir="ltr">
  ...
  <div id="searchesWrapper">
    <div id="searchesHeader"><?php echo(ucfirst($_SESSION['username']));?>'s Searches</div>
    <ul id="searchesList">
      <?php while ( $row = $results->fetch_assoc() ) : ?>
        <li class="searchesListItem hoverEffect" value="<?php echo($row['searchID']);?>"><i class="fas fa-trash-alt delete"></i>
          <a href="index.php?lat=<?php echo($row['lat']);?>&lng=<?php echo($row['lng']);?>&miles=<?php echo($row['miles']);?>"><?php echo($row['city']);?> (<?php echo($row['miles']);?>mi, <?php echo(number_format($row['lat'],5,'.',''));?>, <?php echo(number_format($row['lng'],5,'.',''));?>)</a></li>
  ...
  <script>
    ...
    $(document).on('click','.delete',function(){
      ...
      $obj=$(this).parent();
      $.get("removeFromSearches.php",{search_id:$obj.attr('value')},function(response){
