...
require 'config.php';
	...
	if (!isset($_POST['username']) || empty($_POST['username']) ) {
		//$usernameLoginError="*Username field is empty.";
	}
	if (isset($_POST['username']) && isset($_POST['password']) && !empty($_POST['username']) && !empty($_POST['password']) ) {
	        ...
	        $_SESSION['username'] = null;
					$passwordLoginError = "*Password does not match.";
				}
			} else {
				$usernameLoginError="*Username not found.";
			}
	} else {
		//$passwordLoginError="*Password field is empty.";
	}
?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
  ...
  <div class="logregbox logbox">
      ...
      <em><h6 id="usernameLoginError"><?php if($usernameLoginError!=""){ echo($usernameLoginError); };?></h6></em>
  	  Password: <br>
  	  <input id="password" type="password" name="password"><br>
  	  <em><h6 id="passwordLoginError"><?php if($passwordLoginError!=""){ echo($passwordLoginError); };?></h6></em>
    ...
    var $key="<?php echo(GOOGLE_API_KEY); ?>";
		...
		$(".logregform").on("submit",function(event){
			if($("#username").val()==null || $("#username").val().trim()==""){
					...
					$("#usernameLoginError").text("*Username field is empty.");
			} else {
					$("#usernameLoginError").text("");
			}
			if($("#password").val()==null || $("#password").val().trim()==""){
					event.preventDefault();
					$("#passwordLoginError").text("*Password field is empty.");
			} else {
